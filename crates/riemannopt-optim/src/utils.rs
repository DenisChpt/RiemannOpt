...
use riemannopt_core::{
	core::manifold::Manifold, error::Result, memory::workspace::Workspace, types::Scalar,
};
...
mod tests {
	...
	fn test_workspace_ops() {
		...
		assert!(optimizer
			.workspace
			.get_buffer::<nalgebra::DVector<f64>>(
				riemannopt_core::memory::workspace::BufferId::Gradient
			)
			.is_some());
	}
}
