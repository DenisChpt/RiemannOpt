//! Integration tests for the L-BFGS optimizer
...
#[test]
fn test_lbfgs_on_sphere() -> Result<()> {
    let dim = 10;
    ...
    let mut optimizer = LBFGS::new(LBFGSConfig::new());
...
#[test]
fn test_lbfgs_with_custom_config() -> Result<()> {
    let dim = 10;
    let sphere = Sphere::<f64>::new(dim)?;
    let cost_fn = QuadraticOnSphere::new(dim);
...
#[test]
fn test_lbfgs_without_line_search() -> Result<()> {
    let dim = 5;
    let sphere = Sphere::<f64>::new(dim)?;
    let cost_fn = QuadraticOnSphere::new(dim);
    ...
    // Create optimizer without line search
