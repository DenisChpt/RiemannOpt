...
impl CostFunction<f64> for TraceMinimization {
    ...
    fn cost_and_gradient_alloc(
        &self,
        point: &DMatrix<f64>,
    ) -> ManifoldResult<(f64, DMatrix<f64>)> {
        let ay = &self.a * point;
...
mod numerical_precision {
    ...
    fn gradient_consistency_check() {
        ...
        for i in 0..n {
            ...
            x_plus[i] += eps;
            x_minus[i] -= eps;
            grad_fd[i] = (cost_fn.cost(&x_plus).unwrap() - cost_fn.cost(&x_minus).unwrap())
                / (2.0 * eps);
        }
