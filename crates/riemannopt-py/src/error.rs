	...
	fn from(err: PyRiemannOptError) -> PyErr {
		match err {
			PyRiemannOptError::Manifold(e) => match &e {
				...
				ManifoldError::NumericalError { .. } => NumericalError::new_err(e.to_string()),
				ManifoldError::NotImplemented { .. } => {
					NotImplementedMethodError::new_err(e.to_string())
				}
			},
			PyRiemannOptError::Optimizer(e) => match &e {
				OptimizerError::LineSearchFailed { .. } => LineSearchError::new_err(e.to_string()),
				OptimizerError::MaxIterationsReached { .. } => {
					ConvergenceError::new_err(e.to_string())
				}
				OptimizerError::InvalidConfiguration { .. } => PyValueError::new_err(e.to_string()),
				OptimizerError::ManifoldError(me) => PyRiemannOptError::Manifold(me.clone()).into(),
				OptimizerError::InvalidSearchDirection => LineSearchError::new_err(e.to_string()),
			},
			PyRiemannOptError::Optimization(e) => OptimizationFailedError::new_err(e),
			PyRiemannOptError::ArrayConversion(e) => PyValueError::new_err(e.to_string()),
...
pub fn register_exceptions(module: &Bound<'_, PyModule>) -> PyResult<()> {
	...
	module.add(
		"ManifoldValidationError",
		module.py().get_type::<ManifoldValidationError>(),
	)?;
	module.add(
		"OptimizationFailedError",
		module.py().get_type::<OptimizationFailedError>(),
	)?;
	module.add(
		"ConvergenceError",
	...
	module.add(
		"NotImplementedMethodError",
		module.py().get_type::<NotImplementedMethodError>(),
	)?;
	Ok(())
}
