...
impl PyEuclidean {
	...
	pub fn new(n: usize) -> PyResult<Self> {
		...
		match Euclidean::new(n) {
			...
			Err(e) => Err(PyValueError::new_err(format!(
				"Failed to create Euclidean manifold: {}",
				e
			))),
		}
	}
	...
	pub fn retract<'py>(
		...
		if tangent.len()? != self.inner.dimension() {
			return Err(PyValueError::new_err(format!(
				"Dimension mismatch for {}: expected {}, got {}",
				"tangent",
				self.inner.dimension(),
	...
	pub fn inverse_retract<'py>(
		...
		if other.len()? != self.inner.dimension() {
			return Err(PyValueError::new_err(format!(
				"Dimension mismatch for {}: expected {}, got {}",
				"other",
				self.inner.dimension(),
	...
	pub fn euclidean_to_riemannian_gradient<'py>(
		...
		if euclidean_grad.len()? != self.inner.dimension() {
			return Err(PyValueError::new_err(format!(
				"Dimension mismatch for {}: expected {}, got {}",
				"euclidean_grad",
				self.inner.dimension(),
	...
	pub fn parallel_transport<'py>(
		...
		if from_point.len()? != self.inner.dimension() {
			return Err(PyValueError::new_err(format!(
				"Dimension mismatch for {}: expected {}, got {}",
				"from_point",
				self.inner.dimension(),
		...
		if to_point.len()? != self.inner.dimension() {
			return Err(PyValueError::new_err(format!(
				"Dimension mismatch for {}: expected {}, got {}",
				"to_point",
				self.inner.dimension(),
