...
impl PyProductManifold {
	...
	pub fn project<'py>(&self, py: Python<'py>, point: PyObject) -> PyResult<PyObject> {
		...
		let projected = self
			...
			.iter()
			.zip(tuple.iter())
			.map(|(manifold, component)| manifold.call_method1(py, "project", (component,)))
			.collect::<PyResult<Vec<_>>>()?;
	...
	pub fn exp<'py>(
		...
		let results = self
			...
			.map(|(manifold, (p, v))| manifold.call_method1(py, "exp", (p, v)))
			.collect::<PyResult<Vec<_>>>()?;
	...
	pub fn random_tangent<'py>(&self, py: Python<'py>, point: PyObject) -> PyResult<PyObject> {
		...
		let tangents = self
			...
			.map(|(manifold, pt)| manifold.call_method1(py, "random_tangent", (pt,)))
			.collect::<PyResult<Vec<_>>>()?;
	...
	pub fn project_tangent<'py>(
		...
		let projected = self
			...
			.map(|(manifold, (pt, vec))| manifold.call_method1(py, "project_tangent", (pt, vec)))
			.collect::<PyResult<Vec<_>>>()?;
	...
	pub fn retract<'py>(
		...
		let retracted = self
			...
			.iter()
			.zip(point_tuple.iter().zip(tangent_tuple.iter()))
			.map(|(manifold, (pt, tan))| manifold.call_method1(py, "retract", (pt, tan)))
			.collect::<PyResult<Vec<_>>>()?;
	...
	pub fn inner<'py>(
		...
		for (manifold, ((pt, u_comp), v_comp)) in self
			.manifolds
			.iter()
	...
	pub fn dist<'py>(&self, py: Python<'py>, x: PyObject, y: PyObject) -> PyResult<f64> {
		...
		for (manifold, (x_comp, y_comp)) in self
			.manifolds
			.iter()
