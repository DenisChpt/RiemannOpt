...
impl PySphere {
	...
	pub fn inner(
		...
		// Validate dimensions
		if point_vec.len() != self.dimension
			|| u_vec.len() != self.dimension
			|| v_vec.len() != self.dimension
		{
			return Err(crate::error::dimension_mismatch(
	...
	pub fn distance(
		...
		Ok(self.inner.distance(&x_vec, &y_vec).map_err(to_py_err)?)
	}
	...
	pub fn random_tangent<'py>(
		...
		let mut result = self.inner.random_tangent(&point_vec).map_err(to_py_err)?;
	...
	pub fn parallel_transport<'py>(
		...
		if from_vec.len() != self.dimension
			|| to_vec.len() != self.dimension
			|| tangent_vec.len() != self.dimension
		{
			return Err(crate::error::dimension_mismatch(
				&[self.dimension, self.dimension, self.dimension],
