...
fn test_retraction_convergence_projection() {
    ...
    for &h in &step_sizes {
        ...
        let exp_result = sphere.retract(&point, &scaled_tangent).unwrap();
...
fn test_convergence_small_tangents() {
    ...
    let tangent = sphere.random_tangent(&point).unwrap() * 1e-6;
...
impl Manifold<f64, Dyn> for WeightedEuclidean {
    ...
    fn euclidean_to_riemannian_gradient(
        ...
        let mut riem_grad = grad.clone();
        for i in 0..self.dim {
            riem_grad[i] /= self.weights[i];
        }
        Ok(riem_grad)
    }
