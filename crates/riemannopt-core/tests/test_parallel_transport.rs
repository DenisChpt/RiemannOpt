...
impl Manifold<f64, Dyn> for SphereWithTransport {
    ...
    fn parallel_transport(
        ...
        self.project_tangent(to, &transported, result)
    }
}
...
fn test_transport_identity() {
    ...
    for _ in 0..20 {
        ...
        manifold
            .parallel_transport(&point, &point, &tangent, &mut transported)
            .unwrap();
...
fn test_transport_preserves_norm() {
    ...
    for _ in 0..20 {
        ...
        sphere.random_tangent(&from, &mut tangent).unwrap();
        let mut transported = DVector::zeros(from.len());
        sphere.parallel_transport(&from, &to, &tangent, &mut transported).unwrap();
...
fn test_transport_preserves_inner_products() {
    ...
    for _ in 0..10 {
        ...
        sphere.random_tangent(&from, &mut u).unwrap();
        let mut v = DVector::zeros(from.len());
        sphere.random_tangent(&from, &mut v).unwrap();

        let mut u_transported = DVector::zeros(from.len());
        sphere.parallel_transport(&from, &to, &u, &mut u_transported).unwrap();
        let mut v_transported = DVector::zeros(from.len());
        sphere.parallel_transport(&from, &to, &v, &mut v_transported).unwrap();
...
fn test_transport_linearity() {
    ...
    let mut direction = DVector::zeros(from.len());
    manifold.random_tangent(&from, &mut direction).unwrap();
    direction *= 0.1;
    let mut to = DVector::zeros(from.len());
    manifold.retract(&from, &direction, &mut to).unwrap();

    let mut u = DVector::zeros(from.len());
    manifold.random_tangent(&from, &mut u).unwrap();
    let mut v = DVector::zeros(from.len());
    manifold.random_tangent(&from, &mut v).unwrap();
    let alpha = 2.5;
    let beta = -1.3;
    ...
    manifold.parallel_transport(&from, &to, &combined, &mut transport_combined).unwrap();

    let mut u_transported = DVector::zeros(from.len());
    manifold.parallel_transport(&from, &to, &u, &mut u_transported).unwrap();
    let mut v_transported = DVector::zeros(from.len());
    manifold.parallel_transport(&from, &to, &v, &mut v_transported).unwrap();
...
fn test_transport_zero_vector() {
    ...
    let mut transported = DVector::zeros(from.len());
    manifold.parallel_transport(&from, &to, &zero, &mut transported).unwrap();
...
fn test_transport_inverse_consistency() {
    ...
    for _ in 0..10 {
        ...
        sphere.random_tangent(&point_a, &mut direction).unwrap();
        direction *= 0.05;
        let mut point_b = DVector::zeros(point_a.len());
        sphere.retract(&point_a, &direction, &mut point_b).unwrap();

        let mut tangent = DVector::zeros(point_a.len());
        sphere.random_tangent(&point_a, &mut tangent).unwrap();

        // Transport A -> B -> A
        let mut transported_to_b = DVector::zeros(point_a.len());
        sphere
            .parallel_transport(&point_a, &point_b, &tangent, &mut transported_to_b)
            .unwrap();
        let mut transported_back = DVector::zeros(point_a.len());
        sphere
            .parallel_transport(&point_b, &point_a, &transported_to_b, &mut transported_back)
            .unwrap();
...
fn test_transport_along_geodesic() {
    ...
    sphere.random_tangent(&from, &mut geodesic_tangent).unwrap();
    geodesic_tangent *= 0.1;
    let mut to = DVector::zeros(from.len());
    sphere.retract(&from, &geodesic_tangent, &mut to).unwrap();

    // Transport the geodesic tangent itself
    let mut transported_tangent = DVector::zeros(from.len());
    sphere
        .parallel_transport(&from, &to, &geodesic_tangent, &mut transported_tangent)
        .unwrap();
