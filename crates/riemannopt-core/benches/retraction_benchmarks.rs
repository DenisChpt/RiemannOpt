    ...
    for dim in [10, 50, 100, 500] {
        ...
        let test_data: Vec<(DVector<f64>, DVector<f64>)> = (0..100)
            .map(|_| {
                ...
                manifold.retract(&point, &tangent, &mut other, &mut workspace).unwrap();
                (point, other)
            })
            .collect();
        ...
        group.bench_with_input(BenchmarkId::new("logarithm", dim), &dim, |b, _| {
            ...
            b.iter(|| {
                ...
                let mut workspace = Workspace::<f64>::new();
                manifold
                    .inverse_retract(black_box(point), black_box(other), &mut result, &mut workspace)
                    .unwrap();
                idx += 1;
                black_box(result)
            });
    ...
    group.bench_function("projection_accuracy_check", |b| {
        ...
        b.iter(|| {
            ...
            for (point, tangent) in &test_data {
                ...
                manifold.retract(point, tangent, &mut exact, &mut workspace).unwrap();
