	...
	for i in 0..n {
		// Create unit vector in direction i
		{
			let e_i = workspace
				...
				.ok_or_else(|| {
					crate::error::ManifoldError::invalid_parameter(
						"Failed to get unit vector buffer".to_string(),
					)
				})?;
			e_i.fill(T::zero());
			e_i[i] = T::one();
		...
		// Central difference
		{
			let point_plus = workspace
				...
				.ok_or_else(|| {
					crate::error::ManifoldError::invalid_parameter(
						"Failed to get point plus buffer".to_string(),
					)
				})?;
...

		{
			let point_minus = workspace
				...
				.ok_or_else(|| {
					crate::error::ManifoldError::invalid_parameter(
						"Failed to get point minus buffer".to_string(),
					)
				})?;
	...
	F: Fn(&nalgebra::DVector<T>) -> Result<nalgebra::DVector<T>>,
{
	...
	let perturbed =
		workspace.get_or_create_buffer(BufferId::Temp1, || nalgebra::DVector::<T>::zeros(n));
...
mod tests {
	...
	fn test_hessian_vector_product_workspace() {
		...
		let gradient_fn = |x: &DVector<f64>| -> Result<DVector<f64>> { Ok(&a * x + &b) };

		// Compute Hessian-vector product with workspace
		let mut hvp_workspace = DVector::<f64>::zeros(n);
		hessian_vector_product(&gradient_fn, &x, &v, &mut workspace, &mut hvp_workspace).unwrap();
	...
	fn test_workspace_reuse() {
		...
		assert!(workspace
			.get_buffer::<DVector<f64>>(BufferId::Gradient)
			.is_some());
		assert_eq!(
			workspace
				.get_buffer::<DVector<f64>>(BufferId::Gradient)
