	...
	F: CostFunction<f64, Point = DVector<f64>, TangentVector = DVector<f64>>,
{
	...
	fn cost_and_gradient(
		...
		// Try to read from cache first
		{
			...
			if let Some(ref cache_entry) = *cache_read {
				if cache_entry.matches_with_tolerance(point, self.tolerance) {
					if let (Some(value), Some(ref cached_grad)) =
						(cache_entry.value, &cache_entry.gradient)
					{
	...
	fn cost_and_gradient_alloc(&self, point: &Self::Point) -> Result<(f64, Self::TangentVector)> {
		...
		// Try to read from cache first
		{
			...
			if let Some(ref cache_entry) = *cache_read {
				if cache_entry.matches_with_tolerance(point, self.tolerance) {
					if let (Some(value), Some(ref gradient)) =
						(cache_entry.value, &cache_entry.gradient)
					{
						self.stats.lock().combined_hits += 1;
	...
	fn gradient(&self, point: &Self::Point) -> Result<Self::TangentVector> {
		...
		let (gradient, cost_opt) =
			if let Ok((cost, grad)) = self.inner.cost_and_gradient_alloc(point) {
				(grad, Some(cost))
			} else {
...
impl CacheStatistics {
	...
	pub fn hit_rate(&self) -> f64 {
		...
		let total_accesses =
			total_hits + self.cost_misses + self.gradient_misses + self.combined_misses;
