...
pub trait GpuMatrixOps<T: Scalar> {
	...
	fn elementwise_add(&self, a: &GpuMatrix<T>, b: &GpuMatrix<T>)
		-> Result<GpuMatrix<T>, GpuError>;

	fn elementwise_mul(&self, a: &GpuMatrix<T>, b: &GpuMatrix<T>)
		-> Result<GpuMatrix<T>, GpuError>;
}
...
pub trait GpuBatchOps<T: Scalar> {
	...
	fn batch_project(&self, points: &[GpuMatrix<T>]) -> Result<Vec<GpuMatrix<T>>, GpuError>;
