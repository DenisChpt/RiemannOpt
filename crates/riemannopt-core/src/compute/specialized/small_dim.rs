...
#[inline(always)]
pub fn retract_sphere_2d<T: Scalar>(point: &Vector2<T>, tangent: &Vector2<T>, t: T) -> Vector2<T> {
	// Compute point + t * tangent
	let moved = Vector2::new(point[0] + t * tangent[0], point[1] + t * tangent[1]);
...
#[inline(always)]
pub fn retract_sphere_3d<T: Scalar>(point: &Vector3<T>, tangent: &Vector3<T>, t: T) -> Vector3<T> {
	// Compute point + t * tangent
...
#[inline(always)]
pub fn project_tangent_sphere_2d<T: Scalar>(point: &Vector2<T>, vector: &mut Vector2<T>) {
	// Project: v - <v, p> * p
...
#[inline(always)]
pub fn project_tangent_sphere_3d<T: Scalar>(point: &Vector3<T>, vector: &mut Vector3<T>) {
	// Project: v - <v, p> * p
