...
pub trait SparseMatrixOps<T: Scalar> {
	...
	fn sparse_gemv(&self, matrix: &CsrMatrix<T>, x: &DVector<T>, y: &mut DVector<T>) -> Result<()>;
...
mod tests {
	...
	fn test_sparse_aware_backend() {
		let backend = SparseAwareBackend::<f64>::new().with_threshold(0.5); // Use sparse if >50% zeros
