...
impl ParallelBatch {
	...
	pub fn evaluate<T, F>(points: &PointBatch<T>, func: F) -> Vec<T>
	where
		T: Scalar,
...
impl ParallelSGD {
	...
	pub fn process_batches<T, D, F, R>(data: &[D], batch_size: usize, process_batch: F) -> Vec<R>
	where
		T: Scalar,
		D: Send + Sync,
		F: Fn(&[D]) -> R + Send + Sync,
		R: Send,
	{
		data.par_chunks(batch_size).map(process_batch).collect()
	}

	/// Computes average gradient over mini-batches in parallel.
	pub fn average_gradients<T>(gradients: &[DVector<T>]) -> DVector<T>
	where
		T: Scalar,
	{
		let n = gradients.len();
		...
		let sum: DVector<T> = gradients
			.par_iter()
			.map(|g| g.clone())
			.reduce(|| DVector::zeros(dim), |a, b| a + b);
...
impl ParallelAverage {
	...
	pub fn average_parameters<T>(parameters: &[DVector<T>]) -> DVector<T>
	where
		T: Scalar,
	{
	...
	pub fn weighted_average<T>(parameters: &[DVector<T>], weights: &[T]) -> DVector<T>
	where
		T: Scalar,
	{
