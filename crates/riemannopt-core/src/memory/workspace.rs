...
use crate::{
	memory::pool::{MatrixPool, PooledMatrix, PooledVector, VectorPool},
...
impl<T: Scalar> Workspace<T> {
	...
	pub fn with_size(n: usize) -> Self {
		...
		workspace.buffers.insert(
			BufferId::Gradient,
			Box::new(DVector::<T>::zeros(n)) as Box<dyn WorkspaceBuffer<T>>,
		);
		workspace.buffers.insert(
			BufferId::Direction,
			Box::new(DVector::<T>::zeros(n)) as Box<dyn WorkspaceBuffer<T>>,
	...
	pub fn get_or_create_buffer<B: WorkspaceBuffer<T> + 'static>(
		&mut self,
		id: BufferId,
...
mod tests {
	...
	fn test_workspace_basic() {
		...
		assert!(workspace
			...
			.is_some());
		assert!(workspace
			.get_buffer::<DVector<f64>>(BufferId::Direction)
			.is_some());
	...
	fn test_workspace_builder() {
		...
		assert_eq!(
			workspace
				.get_buffer::<DVector<f32>>(BufferId::Gradient)
				.unwrap()
				.len(),
			20
		);
		assert_eq!(
			workspace
				.get_buffer::<DVector<f32>>(BufferId::Momentum)
				.unwrap()
				.len(),
			20
		);
		assert_eq!(
			workspace
				.get_buffer::<DMatrix<f32>>(BufferId::Hessian)
				.unwrap()
				.nrows(),
			20
		);
	...
	fn test_get_or_create() {
		...
		assert!(workspace
			.get_buffer::<DVector<f64>>(BufferId::Temp3)
			.is_none());
