...
pub fn get_pooled_vector<T: Scalar + 'static>(size: usize) -> PooledVector<T> {
	...
	if TypeId::of::<T>() == TypeId::of::<f32>() {
		F32_VECTOR_POOL.with(|pool| unsafe { std::mem::transmute(pool.acquire(size)) })
	} else if TypeId::of::<T>() == TypeId::of::<f64>() {
		F64_VECTOR_POOL.with(|pool| unsafe { std::mem::transmute(pool.acquire(size)) })
	} else {
...
pub fn get_pooled_matrix<T: Scalar + 'static>(rows: usize, cols: usize) -> PooledMatrix<T> {
	...
	if TypeId::of::<T>() == TypeId::of::<f32>() {
		F32_MATRIX_POOL.with(|pool| unsafe { std::mem::transmute(pool.acquire(rows, cols)) })
	} else if TypeId::of::<T>() == TypeId::of::<f64>() {
		F64_MATRIX_POOL.with(|pool| unsafe { std::mem::transmute(pool.acquire(rows, cols)) })
	} else {
		panic!("Memory pool only supports f32 and f64");
	}
}
