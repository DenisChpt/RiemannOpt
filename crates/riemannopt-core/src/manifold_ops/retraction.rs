...
impl<T: Scalar> VectorTransport<T> for ProjectionTransport {
	fn transport<M: Manifold<T>>(
		...
		let _destination = workspace.get_or_create_buffer(crate::memory::BufferId::Temp1, || {
...
impl<T: Scalar> VectorTransport<T> for ParallelTransport {
	fn transport<M: Manifold<T>>(
		...
		let _destination = workspace
			.get_or_create_buffer(crate::memory::BufferId::Temp1, || DVector::<T>::zeros(0));
...
impl VectorTransportHelper {
	...
	pub fn tangent_vector_distance<T: Scalar, M: Manifold<T>>(
		...
		Ok(<T as num_traits::Float>::sqrt(num_traits::Float::max(
			difference_norm_sq,
			T::zero(),
		)))
	}
