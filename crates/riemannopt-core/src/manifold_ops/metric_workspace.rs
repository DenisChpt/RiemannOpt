...
impl VectorMetricWorkspace {
	...
	pub fn norm<T: Scalar>(v: &DVector<T>, workspace: &mut Workspace<T>) -> Result<T> {
	...
	pub fn gram_matrix<T: Scalar>(
		...
		assert_eq!(
			result.nrows(),
			n,
			"Gram matrix must have correct dimensions"
		);
...
impl MatrixMetricWorkspace {
	...
	pub fn frobenius_norm<T: Scalar>(m: &DMatrix<T>, workspace: &mut Workspace<T>) -> Result<T> {
...
mod tests {
	...
	fn test_weighted_inner_product() {
		...
		let inner =
			MatrixMetricWorkspace::weighted_inner_product(&a, &b, &w, &mut workspace).unwrap();
		assert_eq!(inner, 5.0);
	}
