...
fn test_spd_projection() {
    ...
    assert_relative_eq!(&proj - proj.transpose(), DMatrix::zeros(3, 3), epsilon = 1e-14);
...
fn test_spd_euclidean_to_riemannian_gradient() {
    ...
    spd.euclidean_to_riemannian_gradient(&p, &egrad, &mut rgrad).unwrap();
