#!/usr/bin/env python3
"""
Corrected PyManopt benchmark with proper API usage.
"""
    ...
    def benchmark_sphere(self):
        ...
        for n in sizes:
            ...
            try:
                ...
                stats = self.time_operation(manifold.projx, x)
                ...
                x_on_sphere = manifold.projx(x)
                v = np.random.randn(n)
                
                # Benchmark tangent projection
                stats = self.time_operation(manifold.proju, x_on_sphere, v)
                ...
                # Get tangent vector
                v_tangent = manifold.proju(x_on_sphere, v)
    ...
    def benchmark_stiefel(self):
        ...
        for n, p in sizes:
            ...
            try:
                ...
                # Get point on manifold
                X_on_stiefel = manifold.projx(X)
                V = np.random.randn(n, p)
                
                # Benchmark tangent projection
                stats = self.time_operation(manifold.proju, X_on_stiefel, V)
                print(f"Tangent projection: {stats['mean']*1000:.3f} ± {stats['std']*1000:.3f} ms")
                self.results.append({
                    'manifold': 'stiefel',
                    'operation': 'tangent_projection',
                ...
                # Get tangent vector
                V_tangent = manifold.proju(X_on_stiefel, V)
